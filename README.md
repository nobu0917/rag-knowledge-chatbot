...
```mermaid
flowchart LR
  subgraph ingest[インジェスト（事前処理: npm run ingest）]
    MD[規程Markdown] --> CH[チャンク分割<br>見出し単位] --> EMB1[Gemini埋め込みAPI<br>gemini-embedding-001] --> JSON[(embeddings.json)]
  end
```
...
- **PDF対応**: 現在はMarkdownのみ。PDF取り込み（ページ番号付き出典）は `Source` 型の拡張で対応予定
